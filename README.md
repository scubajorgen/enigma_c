...
The core of the software is mainly intended for research and to be used to break Enigma encoded ciphers and study its workings. For this the software is performance optimized. It is intended for **software devolopers who are familiar with the C language**. To show its workings, a lot of examples are provide which can be run from the menu. It is not intended as some stand alone end user tool. For this, there are plenty of tools online, for example [cryptii](https://cryptii.com/pipes/enigma-machine).
...
* **testPerformanceEngima** This runs the performance test of the software by executing 1.000.000 encodes/decodes for various cipher lengths. It takes various minutes, depending on your PC. It returns csv output.
* **testPerformanceIoc** This performs an effectiveness test of the James Gillogly IoC method. It takes various hours. It returns csv output.
...
./test
```
This should result in a lot of output, ending with following (especially no tests failed):
```
INFO    ###############################################################################
...
For a 1000 character message a decode takes 22.3 μs on the N100, hence 44.800 decodes/s. The i5-12500H is the winner with 12.3 μs such a message, meaning 81.300 decods/s. All when executed in one thread.

We can speed up things, by using multithreading, using the WorkDispatcher. The effect of it is shown in next chart for a 1000 character message, on the i5-12500H (12 cores, 16 virtual processors).

![](images/encodeDecodePerformanceThreads.png)

Up till 4 threads performance increases linearly with the number of threads. Up till 10 threads we see another lineair increase, but at a less steeper rate. The maximum is reached with 21 threads: a stunning 608.000 decodes per second (=740%). Given 12 cores each running one thread, we would expect 1200% at 12 threads, but alas.
...
Generally, computerized code cracking tries various permutations of rotors, rotorsettings and plugs and measures to what extent the result 'approaches' readable text. If you try all possible permutations, this is called 'brute forcing' and this takes an awful lot of time or might even be impossible. So you have to do better than that.

First of all we need a automatic measure that indicates deciphered text approaches readable text. For example:
...
The Gillogly method consists of following steps:
...
* At step 1, take not one candidate (that scores best) as _the_ solution, but take a number of best scoring candidates. Then execute step 2 on all found solutions.
...
The software calculates the permutations of rotors (when NULL is passed as second parameter), given the type of enigmaType and the rotorSet. Alternatively, the user can define his own permutations and provide it to iocDecodeText() (refer to example code):
...
### Performance
First the performance. Next diagram shows the time for decoding and evaluation of the result wihth IoC.
...
The red line adds setting up the Enigma (Walzen, Ringstellungen, Steckers, ...) and reading out the result. The added time also increases with number of characters in the cipher, since it includes creating the result as string (toString() function).

The grey line adds the IoC calculation. 

We see that by far the calucalations for decode are the main component. Calculation of the IoC is relatively cheap.
...
1. Choose text length
    1. Choose number of plugs (0-12)
        1. Repeat 50 times
            ...
            4. Crack cipher text: find best solution
            5. Measure overlap between plaintext and cracked cipher (percentage of characters that is the same)
...
Whereas the Gillogly requires large ciphers, this ngram method performs better for shorter ciphers. The method is implemented in ```ngramScore.c```.
